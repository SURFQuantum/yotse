...
class Optimizer:
    ...
    def __init__(self, optimization_algorithm: GenericOptimization):
        ...
        if optimization_algorithm is not None:
            if not isinstance(optimization_algorithm, GenericOptimization):
                raise ValueError(
                    "Optimization algorithm must be subclass of GenericOptimization."
                )
    ...
    def construct_points(
        self, experiment: Experiment, evolutionary: bool, points_per_param=None
    ) -> None:
        """
    ...
    def grid_based_point_creation(
        self, experiment: Experiment, points_per_param: int
    ) -> None:
