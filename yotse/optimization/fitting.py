...
class FuncFit:
    ...
    def add_noise(self, arr, level=0.2):
        """
        Add noise to the given array
        :param arr: Array of values (numpy array)
        :param level: Noise level
        :return: Input array with added noise
        """
        return np.random.normal(2*arr+2, level)

    def chi_square(self, poly, x, y):
        """
        Calculate chi-square
        :param poly: Polynomial (numpy.poly1d object)
        :param x: X-points of the original dataset
        :param y: Y-points of the original dataset
        :return: Chi-square
        """
        return np.sum((np.polyval(poly, x) - y) ** 2) / len(x)

    def err(self, poly, x, y):
        """
        Calculate error
        :param poly: Polynomial (numpy.poly1d object)
        :param x: X-points of the original dataset
    ...
    def find_poly_fit(self, x, y, plt=None, max_order=41):
        """
        Find a polynomial that fits the given data best.
        :param x: X-points
        :param y: Y-points
        :param max_order: Maximum order of a polynomial
        :return: A list of all polynomials and the corresponding errors. The list is sorted in
                 ascending order using the error values.
        """
        poly_fits = []
