...
class ModGA(pygad.GA):
    ...
    def run(self):
        ...
        if self.generations_completed != 0 and type(self.generations_completed) in ModGA.supported_int_types:
        ...
        for generation in range(generation_first_idx, generation_last_idx):
                ...
                if not type(self.last_generation_parents) is numpy.ndarray:
                    raise TypeError(
                        ...
                        "{last_generation_parents_type} found.".format(
                            last_generation_parents_type=type(self.last_generation_parents)))
                if not type(self.last_generation_parents_indices) is numpy.ndarray:
                    raise TypeError(
                        ...
                        "(numpy.ndarray) but {last_generation_parents_indices_type} found.".format(
                            last_generation_parents_indices_type=type(self.last_generation_parents_indices)))
            else:
                self.last_generation_parents, self.last_generation_parents_indices = self.select_parents(
                ...
                if self.last_generation_parents.shape[0] != self.num_parents_mating:
                    raise ValueError(
                        ...
                        "({num_parents_mating_selected}) selected.".format(
                            ...
                            parents_size_expected=(self.num_parents_mating, self.num_genes),
                            num_parents_mating=self.num_parents_mating,
                            num_parents_mating_selected=self.last_generation_parents.shape[0]))
                    ...
                    raise ValueError(
                        ...
                        "({num_genes_selected}) produced.".format(
                            parents_size_actual=self.last_generation_parents.shape,
                            parents_size_expected=(self.num_parents_mating, self.num_genes), num_genes=self.num_genes,
            ...
            if self.last_generation_parents_indices.ndim != 1:
                ...
                raise ValueError(
                    ...
                    "values but ({num_parents_mating_selected}) found.".format(
                        num_parents_mating=self.num_parents_mating,
                        num_parents_mating_selected=len(self.last_generation_parents_indices)))
            ...
            if self.crossover_type is None:
                if self.keep_elitism == 0:
                    ...
                    if self.num_offspring <= num_parents_to_keep:
                        ...
                        self.last_generation_offspring_crossover = \
                            numpy.concatenate((self.last_generation_parents,
                                               self.population[0:(self.num_offspring -
                                                                  self.last_generation_parents.shape[0])]))
                else:
                    # The steady_state_selection() method is called to select the best solutions (i.e. elitism).
                    # The keep_elitism parameter defines the number of these solutions.
                    # The steady_state_selection() method is still called here even if its output may not be used given
                    # that the condition of the next if statement is True. The reason is that it will be used later.
                    ...
                    if self.num_offspring <= self.keep_elitism:
                        ...
                        self.last_generation_offspring_crossover = \
                            numpy.concatenate((self.last_generation_elitism,
                                               self.population[0:(self.num_offspring -
                ...
                if callable(self.crossover_type):
                    ...
                    self.last_generation_offspring_crossover = self.crossover(self.last_generation_parents,
                                                                              offspring_size=(self.num_offspring,
                                                                                              self.num_genes))
                    ...
                    if self.last_generation_offspring_crossover.shape[0] != self.num_offspring:
                        raise ValueError(
                            ...
                            "offspring but ({num_offspring_produced}) produced.".format(
                                ...
                                crossover_expected_size=(self.num_offspring, self.num_genes),
                                num_offspring=self.num_offspring,
                                num_offspring_produced=self.last_generation_offspring_crossover.shape[0]))
                        ...
                        raise ValueError(
                            ...
                            "({num_genes}) genes but ({num_genes_produced}) produced.".format(
                                crossover_actual_size=self.last_generation_offspring_crossover.shape,
                                crossover_expected_size=(self.num_offspring, self.num_genes), num_genes=self.num_genes,
                                num_genes_produced=self.last_generation_offspring_crossover.shape[1]))
                    ...
                    if not type(self.last_generation_offspring_mutation) is numpy.ndarray:
                        raise TypeError(
                            ...
                            "{last_generation_offspring_mutation_type} found.".format(
                                last_generation_offspring_mutation_type=type(self.last_generation_offspring_mutation)))
                else:
                    self.last_generation_offspring_mutation = self.mutation(self.last_generation_offspring_crossover)
                    ...
                    if self.last_generation_offspring_mutation.shape[0] != self.num_offspring:
                        raise ValueError(
                            ...
                            "but ({num_offspring_produced}) produced.".format(
                                ...
                                mutation_expected_size=(self.num_offspring, self.num_genes),
                                num_offspring=self.num_offspring,
                                num_offspring_produced=self.last_generation_offspring_mutation.shape[0]))
                        ...
                        raise ValueError(
                            ...
                            "genes but ({num_genes_produced}) produced.".format(
                                mutation_actual_size=self.last_generation_offspring_mutation.shape,
                                mutation_expected_size=(self.num_offspring, self.num_genes), num_genes=self.num_genes,
                                num_genes_produced=self.last_generation_offspring_mutation.shape[1]))
            ...
            if self.keep_elitism == 0:
                ...
                self.last_generation_elitism, self.last_generation_elitism_indices = self.steady_state_selection(
            ...
            self.generations_completed = generation + 1  # The generations_completed attribute holds the number of the
            ...
            # #### Modified past this line:
            # since we are not interested in the fitness of the latest generation nor the solution just skip this
            # self.last_generation_fitness = self.cal_pop_fitness()
            # best_solution, best_solution_fitness, best_match_idx =
            # self.best_solution(pop_fitness=self.last_generation_fitness)
                ...
                for criterion in self.stop_criteria:
                    if criterion[0] == "reach":
                        ...
                        if (self.generations_completed >= criterion[1]):
                            if (self.best_solutions_fitness[
                                    self.generations_completed -
                                    criterion[1]] - self.best_solutions_fitness[self.generations_completed - 1]) == 0:
        ...
        self.best_solution_generation = \
            numpy.where(numpy.array(self.best_solutions_fitness) ==
                        numpy.max(numpy.array(self.best_solutions_fitness)))[0][0]
