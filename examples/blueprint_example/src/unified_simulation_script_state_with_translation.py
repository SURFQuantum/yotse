"""This script takes as input a configuration file and a parameter file. You can find
examples of these for each of the platforms we simulate in the same folder. It parses
these files using netconf and runs an end-to-end entanglement generation experiment
using a link layer protocol. It then stores this data in a RepChainDataFrameHolder
object and saves it in a pickle file. The script is capable of detecting if a parameter
is being varied in a configuration file and, using the netconf snippet, run the
simulation for each of the values of this parameter.

Through optional input arguments one can define the path where the results should be
...
def find_varied_param(generator):
    """Determines which (if any) parameter is being varied in the simulation by
    comparing the configurations generated by netconf.
...
def run_simulation(
    ...
    """Places a request for a given number of pairs to be created and kept, runs the
    simulation and collects the resulting data.
...
def save_data(meas_holder, args):
    """Saves the repchain dataframe holder with the simulation data to the specified
    output path.
...
def collect_state_data(
    ...
    """Runs simulation and collects data for each network configuration. Stores data in
    RepchainDataFrameHolder in format that allows for plotting at later point.
